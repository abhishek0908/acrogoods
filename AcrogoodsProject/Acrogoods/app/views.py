...
def add_to_cart(request):
    user = request.user  # Declare the 'user' variable here

    if request.user.is_authenticated:
        product_id = request.GET.get('prod_id')
        product = Product.objects.get(id=product_id)
        reg = Cart(user=user, product=product).save()
...
def mobile(request,data=None):
    ...
    if data==None:
        mobiles = Product.objects.filter(category = 'M')
    elif data=='Samsung' or data == 'Realme'or data == 'Apple'or data == 'POCO'or data == 'Motorola':
...
def checkout(request):
      ...
      if cart_product:
         for p in cart_product:
               ...
               amount+=tempamount
         total_amount+=amount+shipping_charges   
      return render(request, 'app/checkout.html',{'add':add,'totalamount':total_amount,'cart_item':cart_items,'totalitem':totalitem})
